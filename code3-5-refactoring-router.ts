...
http
  .createServer((req: any, res: any) => {
    ...
    if (path in urlMap) {
      (urlMap[path] as (req: any, res: any) => void)(req, res);
    } else {
      notFound(req, res);
    }
  })
...
const user = (req: any, res: any) => {
  const userInfo: any = url.parse(req.url, true).query;
...
const urlMap: Record<string, (req: any, res: any) => void> = {
  "/": (req: any, res: any) => res.end("HOME"),
  "/user": user,
  "/feed": feed,
};
